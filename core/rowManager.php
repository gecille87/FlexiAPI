...
function deleteTableRow1($input)
{
    // 1. Input validation
    ...
    // 4. Build dynamic placeholders (?, ?, ?...) and prepare query
    $placeholders = implode(',', array_fill(0, count($values), '?'));
    $types = str_repeat('s', count($values)); // assume string by default

    foreach ($values as $v) {
        if (!is_scalar($v)) {
            jsonResponse(false, "Invalid value type in 'values' array.");
        }
    }
    ...
    if (!$stmt) {
        jsonResponse(false, "Failed to prepare delete statement.", ['error' => $conn->error]);
    }

    // bind_param needs variables passed by reference
    $params = [];
    ...
    if (!$stmt->execute()) {
        jsonResponse(false, "Failed to delete row(s).", ['error' => $stmt->error]);
    }
    ...
    if ($affected === 0) {
        jsonResponse(false, "No rows deleted. Records may not exist.");
    }

    jsonResponse(true, "Successfully deleted $affected row(s).");
}
